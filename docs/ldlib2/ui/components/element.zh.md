...
从概念上讲，它类似于 HTML 中的 `#!html <div/>` 元素：一个通用容器，可以设置样式、布局，并扩展行为。
...
在使用前，建议先阅读 [Layout](../preliminary/layout.md){ data-preview }。
...
!!! note ""
    要使用 grid 布局，请在容器元素上设置 `display(TaffyDisplay.GRID)`。模板属性在**容器**上定义网格结构，而 `grid-row` 和 `grid-column` 放在**子元素**上以控制它们的位置。
...
!!! info ""
    ...
    控制**子元素**在网格容器中的行位置。在子元素上设置，而不是容器上。
...
!!! info ""
    ...
    控制**子元素**在网格容器中的列位置。使用与 `grid-row` 相同的放置语法。
...
!!! info ""
    ...
    === "LSS"
        查看 [Texture in LSS](../textures/lss.md) 了解 LSS 支持。
...
从语义上讲，不允许直接添加、删除或重新排序内部元素。
但是，你仍然可以通过编辑器或 XML 编辑它们的样式和管理它们的子元素。
在编辑器中，内部元素在层级视图中以灰色显示。
...
| `animation()`                | `StyleAnimation`                             | 创建针对此元素的样式动画。参见 [StyleAnimation](../preliminary/style_animation.md){ data-preview }。|
...
服务器端事件监听器在**服务器**而不是客户端上运行。它们使用相同的 `UIEvents` 类型常量，并支持冒泡和捕获阶段。它们通过内部 RPC 机制自动同步。
