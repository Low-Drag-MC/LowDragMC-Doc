...
1. 创建 `UI` 组件与布局 (1)
    ...
    1.  <DocIcon name="material-hexagon-multiple" /> 创建 `Button` 和 `Item Slot`，设置它们的位置……

2. 绑定 `UI` 功能逻辑 (1)
    { .annotate }

    1.  <DocIcon name="material-hexagon-multiple" /> 添加 `Button` 被点击时执行的逻辑，将 `Inventory` 绑定到 `Item Slot`……

3. 显示 `UI` (1)
    { .annotate }

    1.  <DocIcon name="material-hexagon-multiple" /> 右键物品时打开 `GUI`，右击方块时打开 `GUI`……

从技术上讲，`UI` 功能绑定和 `UI` 组件创建的过程是同时进行的，因为许多控件都提供了在创建时就绑定功能的构造函数。

在本页中，为了更清晰地展示 `UI` 的工作原理，我们在代码示例中**将步骤 1 和步骤 2**分开讲解。

---

## 创建 `UI` 组件与布局

让我们从 [`WidgetGroup`](../widget/WidgetGroup.md) 开始，它是一个用于容纳子组件的容器。因此，我们创建一个 `WidgetGroup` 作为根组件。在[此处](../widget/index.md)查看所有组件。
...
创建 `UI` 后，我们需要实现其逻辑。例如，我们希望**点击 `Button` 来更改 `Label` 的文本**。
