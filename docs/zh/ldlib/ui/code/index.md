...
1. 创建 `UI` 组件与布局 (1)
    ...
    1.  :material-hexagon-multiple: 创建 `Button` 和 `Item Slot`，设置它们的位置……

2. 绑定 `UI` 功能逻辑 (1)
    { .annotate }

    1.  :material-hexagon-multiple: 添加 `Button` 被点击时执行的逻辑，将 `Inventory` 绑定到 `Item Slot`……

3. 显示 `UI` (1)
    { .annotate }

    1.  :material-hexagon-multiple: 右键物品时打开 `GUI`，右击方块时打开 `GUI`……
