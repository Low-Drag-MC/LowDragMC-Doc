...
1.  **数据应该在何时同步？** (每 tick？变化时？打开 GUI 时？)
2.  **应该同步哪些数据？** (哪些字段需要处理？)
3.  **应该如何序列化或保存？** (nbt io?)
...
使用 vanilla Forge 工作流时，如果你想优化同步——例如**仅同步选定的字段**，或**仅同步已更改的字段**——你最终往往会编写更复杂的代码：
...
如果你想要**客户端 → 服务端**同步，则必须创建并注册你自己的网络数据包。
...
- **现代双向 RPC**
  你可以使用 LDLib2 内置的 RPC 事件系统进行**客户端 → 服务端**或**服务端 → 客户端**数据传输，而无需编写数据包。
- **后台（异步）序列化**
...
- 创建 RPC 事件
- 执行手动（可选）细粒度同步
- 将 LDLib2 与 BlockEntity、Entity 和 GUI 系统集成
