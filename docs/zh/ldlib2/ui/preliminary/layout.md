...
LDLib2 UI 布局构建于 [Taffy 布局引擎](https://github.com/DioxusLabs/taffy) 之上。它目前实现了 CSS Block、Flexbox 以及 CSS Grid 布局算法。

> **Taffy** 是一个可嵌入的布局系统，被众多流行 UI 框架所使用。 
> 它**不是** UI 框架，也不执行任何渲染。 
> 它的唯一职责是计算元素的**大小和位置**。

LDLib2 采用 **FlexBox / Grid 布局模型**，让你能够以灵活且可预期的方式描述复杂的 UI 结构。
...
除了下面的示例外，布局属性还可以通过  
[LSS（LDLib Style Sheet）](./stylesheet.md) 定义，这对于将布局逻辑与 UI 结构分离尤其有用。
...
::: info
如果你已经熟悉 Flexbox，Taffy 布局应该会让你感到非常直观。  
如果不熟悉，我们建议阅读官方的 [Taffy 文档](https://taffylayout.com/docs) 以获取完整说明。
...
<figcaption>上图中的蓝色 UI 元素采用 Relative 定位，父元素使用 Flex Direction: Row 作为 Flex 设置。下图中的蓝色 UI 元素采用 Absolute 定位，并忽略父元素的 Flexbox 规则。</figcaption>
</figure>
...
<figcaption>
在此示例中，我们假设根容器 &lt;code&gt;width: 200px&lt;/code&gt;。  
最右侧的元素设置 &lt;code&gt;width: 50%&lt;/code&gt;。因此，剩余空间为 100px，供最左侧和中间元素使用。根据它们的 &lt;code&gt;flex&lt;/code&gt;，它们以 &lt;code&gt;2:1&lt;/code&gt; 的比例分配剩余空间。  
</figcaption>
</figure>
...
<figcaption>
父元素和子 UI 元素使用 Relative 定位以及不同的 Direction 和 Wrap 组合。
</figcaption>
</figure>
...
Grid 类似于 [css](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/Properties/grid)。你可以创建网格布局并对其子元素进行定位。
