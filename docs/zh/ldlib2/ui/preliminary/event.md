...
- `Capture Phase`：在向下捕获阶段发送给元素的事件。
- `Bubbles Phase`：在向上冒泡阶段发送给元素的事件。
...
| 事件 | 描述 | Capture down | Bubbles up | Support Server |
| ----- | ----------- | ------------ | ---------- | ---------- |
...
当 `focusable` 元素获得或失去焦点时，会派发焦点事件。
