...
<VersionBadge version="2.1.6" label="自" icon="tag" />
...
<figure>
<img src="./assets/xml_path.png" alt="编辑器外观" width="80%">
</figure>
...
当 `&lt;stylesheet&gt;` 直接置于 `#!xml &lt;ldlib2-ui&gt;` 下时，它是作用于整个 UI 的**全局样式表**。
...
* **`&lt;stylesheet&gt;`** → 加载外部样式
* **`&lt;style&gt;`** → 定义嵌入式样式
* **元素级别的 `&lt;stylesheet&gt;` / `&lt;style&gt;`** → 仅作用于该元素子树的本地样式表
* **`style` 属性** → 内联样式
* **XML 层级结构** → UI 组件树
