...
<VersionBadge version="2.2.7" label="Since" icon="tag" />
...
If a type is **read-only** (see [Type Support](../../sync/types_support.md)):
...
::: details The correct approach is
...
::: info Before 2.2.6
In versions before `2.2.6`, a common workaround for server-initiated UI actions was to use the generic [RPC Packet](../../sync/rpc_packet.md) system and then manually locate the target UI on the client.
